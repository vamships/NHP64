...
dir_res = paste('results_coxPred/',sep="")
dir.create(dir_res)
...
thresh_upf = 0.75 # polyserial coefficient threshold
...
top_feat_thresh = 95 # Frequency cutoff to determine most-frequent features
...
xtra = read.csv('in/xtra.csv',header=T,row.names=1)

xcolors = matrix("thistle1",nrow=ncol(xtra),ncol=2)

func_tp_6 = read.csv('in/func_tp_6.csv',header=T,row.names=1)

fcolors = matrix("forestgreen",nrow=ncol(func_tp_6),ncol=2)

# -----------------
# Combine Features
# -----------------

feats_original = cbind(func_tp_6,xtra)
lcolors_all = rbind(fcolors,xcolors)
...
if(featSelectMethod=='upf'){
  ...
  selected_feat_idx = univariatePolyserialFilter(thresh_upf,feats_original,subjects,scolors,lcolors_all,plots=T,dir_featSel)
...
feats = data.frame(feats_original[,selected_feat_idx])
lcolors_original = lcolors_all[selected_feat_idx,]
...
for(testIdx in 1:num_repeat){
  ...
  sample_size = nrow(feats_original)
  
  perm_order = sample(sample_size)
  feats_perm = data.frame(feats_original[perm_order,])
  ...
  if(featSelectMethod=='upf'){
    ...
    selected_feat_idx = univariatePolyserialFilter(thresh_upf,feats_perm,subjects,scolors,lcolors_all,plots=FALSE,dir_featSel)
  ...
  lcolors_perm = lcolors_all[selected_feat_idx,]
